...
create table s_dm_variabelerente.rekening_klant_segment
as (
select distinct 
 rek.contract_oid
...
and blk.blokkeringsreden_cd = 23 --- opname stop
...
left join  (select t.contract_oid,t.rekeningsoort_nr --- contracten met wisseling van rekeningsoort
                  ,sum(t.rekeningsoort_nr) over (partition by t.contract_oid order by t.trx_journaal_dt rows between 1 preceding and 1 preceding) as vorige_record
			from  S_DM_VariabeleRente.financiele_transacties t
			qualify sum(t.rekeningsoort_nr) over (partition by t.contract_oid order by t.trx_journaal_dt rows between 1 preceding and 1 preceding)<> t.rekeningsoort_nr
			)trx
...
from s_dm_variabelerente.rekening_klant_segment r
...
join (select contract_oid from   S_DM_VariabeleRente.rekening
      minus
      select contract_oid from s_dm_variabelerente.rekening_klant_segment
)rek_overig
...
delete from s_dm_variabelerente.rekening_klant_segment 
