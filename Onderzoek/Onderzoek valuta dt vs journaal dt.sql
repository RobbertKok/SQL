drop table s_dm_variabelerente.rekening_gem_saldo_jd;

create table s_dm_variabelerente.rekening_gem_saldo_jd
as
(
select contract_oid
...
,zeroifnull(teller/nullifzero(noemer)) as gem_maand_saldo_bg
from 
...
(select
	...
	,saldo_valuta_dt
	, case when dd.aantal_dagen_tot_verval_rte_dag - max(dd.aantal_dagen_tot_verval_rte_dag) OVER (partition by contract_oid,trx_journaal_dt/100 order by trx_journaal_dt rows between 1 following  and 1 following) is null 
           then 	dd.aantal_dagen_tot_verval_rte_dag
           else dd.aantal_dagen_tot_verval_rte_dag - max(dd.aantal_dagen_tot_verval_rte_dag) OVER (partition by contract_oid,trx_journaal_dt/100 order by trx_journaal_dt rows between 1 following  and 1 following)
      end  as dagen_saldo_tussen_maandstanden     
       
 from (select 	row_number() over (partition by trx.contract_oid order by trx.trx_journaal_dt,volg_nr ) as row_nr
				,rek.contract_oid
				,trx.rekeningsoort_nr
				,trx.trx_journaal_dt as trx_journaal_dt
		...
		qualify row_number() over (partition by trx.contract_oid,trx_journaal_dt order by volg_nr desc) = 1 
		)s
...
drop table s_dm_variabelerente.rekening_compensatie_gem_saldo_jd;

create table s_dm_variabelerente.rekening_compensatie_gem_saldo_jd
as(select 
 peil_dt
,contract_oid
,opening_dt
...
(select
	...
	select dim_datum_id as peil_dt
		...
		from 
			...
			and rk.geldig_ind = 1
			left join s_dm_variabelerente.rekening_gem_saldo_jd rgs
...
select a.*,c.*
 from  s_dm_variabelerente.rekening_compensatie_gem_saldo_jd a
join s_dm_variabelerente.rekening_klant_segment b
on a.contract_oid = b.contract_oid
