...
drop table s_dm_variabelerente.rekening_compensatie_gem_saldo1;

create table s_dm_variabelerente.rekening_compensatie_gem_saldo1
as(select 
 peil_dt
,contract_oid
,opening_dt
...
,gecorrigeerde_avg_klant_rte_jaar_perc
,delta
,compensatie_mnd_bg
,cum_compensatie_bg
...
,cum_compensatie_ropr_bg
,compensatie_ropr1_mnd_bg
,sum(compensatie_ropr1_mnd_bg) OVER (partition by contract_oid order by peil_dt rows unbounded preceding ) as cum_compensatie_ropr1_bg
,(compensatie_mnd_bg+compensatie_ropc_mnd_bg+compensatie_ropr_mnd_bg+compensatie_ropr1_mnd_bg) as compensatie_totaal_mnd_bg
,cum_compensatie_bg + cum_compensatie_ropc_bg + cum_compensatie_ropr_bg+ cum_compensatie_ropr1_bg as cum_compensatie_totaal_bg
...
(select
	...
	,((cum_compensatie_ropr_bg - compensatie_ropr_mnd_bg)/12.000) * (gecorrigeerde_avg_klant_rte_jaar_perc/100) as compensatie_ropr1_mnd_bg
	...
	,((cum_compensatie_ropc_bg - compensatie_ropc_mnd_bg)/12.000) * (gecorrigeerde_avg_klant_rte_jaar_perc/100) as compensatie_ropr_mnd_bg
	,compensatie_totaal_mnd_bg
	...
	select dim_datum_id as peil_dt
		...
		,klant_rte_jaar_perc
		,avg_klant_rte_jaar_perc 
		,(avg_klant_rte_jaar_perc + delta) as gecorrigeerde_avg_klant_rte_jaar_perc
		,marge
		,avg_referentie_rte_jaar_perc
		,delta
		...
		,((cum_compensatie_bg - compensatie_mnd_bg)/12.000) * (gecorrigeerde_avg_klant_rte_jaar_perc/100) as compensatie_ropc_mnd_bg
		...
		from 
			...
			,zeroifnull(cast(((delta * rgs.gem_maand_saldo_bg) /12) as decimal(30,2))) as compensatie_mnd_bg
...
-- filter van de records aan het einde van een contract waarbij het saldo = 0 en er geen gebruik meer wordt gemaakt van he DK
-- zo wordt de RopR cumulatie gestopt bij beeindiging ebruik DK
...
from
	(select t.* 
	 ,case when vor_mnd_saldo_bg <> 0 then row_number() over (partition by contract_oid order by peil_dt) else 0 end as rij_nr_vor_saldo
	 ,row_number() over (partition by contract_oid order by peil_dt)  as rij_nr
	 from 
			(select
			a.*
			,sum(a.gem_maand_saldo_bg) over(partition by contract_oid order by peil_dt rows between 1 preceding and 1 preceding) as vor_mnd_saldo_bg
			from  s_dm_variabelerente.rekening_compensatie_gem_saldo1 a
			where 1=1 
...
qualify  rij_nr <= max(rij_nr_vor_saldo) over (partition by contract_oid)
--and vor_mnd_saldo_bg <> 0
)with data unique primary index(contract_oid,peil_dt)
; 
...
-- In 1 keer opbouwen vanuit de basis tabel s_dm_variabelerente.rekening_compensatie_gem_saldo1 
...
,t2.opening_dt
,t2.opgeheven_dt
,t2.cum_compensatie_bg
,t2.cum_compensatie_ropc_bg
,t2.cum_compensatie_ropr_bg
,t2.cum_compensatie_ropr1_bg
,t2.cum_compensatie_totaal_bg
...
qualify max(rij_nr_vor_saldo) over (partition by contract_oid) = rij_nr_vor_saldo and vor_mnd_saldo_bg <> 0
)with data unique primary index(contract_oid)
; 
