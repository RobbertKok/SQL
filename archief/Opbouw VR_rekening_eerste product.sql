...
drop table s_dm_cdo.vr_rekening_first_RSC;

create table s_dm_cdo.vr_rekening_first_RSC
as
(
select 
base.contract_oid
...
,(maandperc_huidige_rsc_bij_afsluiten_contract + zeroifnull(pi2.deb_perc)) as aanvangs_rte_maand_perc
from edv_sas.brek rk
join (select
	 contract_oid
	 ,begin_dt
	 ,product_oid
	from dwh.contract_product_l 
	qualify row_number() over (partition by contract_oid order by eind_dt, geldig_ind desc) = 1  -- eerste bpro_oid = product_oid
	 )cp
on rk.brek_oid = cp.contract_oid
and rk.dwh_eind_dt is null
join edv_sas.bpro pr 
on pr.bpro_oid=cp.product_oid
and pr.aktueel=1 
and pr.dlk=1
and pr.dat_effektief <= rk.dat_opening 
and pr.dat_afloop>rk.dat_opening
--and  cast(pr.dat_effektief -19000000 as date) <= cp.begin_dt
--and cast(pr.dat_afloop -19000000 as date) > cp.begin_dt
and pr.dwh_eind_dt is null
and pr.geldig_ind = 1
join edv_sas.bpre_type5 			pi 
on pi.bank_nr=rk.bank_nr 
and pi.bpro_oid=cp.product_oid 
and pi.pre_grp=5 
