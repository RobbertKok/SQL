...
-- UNDER CONSTRUCTION!!!!!!!
-- gewogen gemiddelde obv volume en aantal dagen

drop table s_dm_cdo.rekening_gem_rente;

create table s_dm_cdo.rekening_gem_rente
as
(
select contract_oid
,laatste_dag_vd_maand  as peil_dt
,sum(klant_rte_jaar_perc*rentedagen_tov_einde_maand) as teller
,sum(rentedagen_tov_einde_maand) as noemer
...
(select
	...
	,klant_rte_jaar_perc
	, case when dd.aantal_dagen_tot_verval_rte_dag - max(dd.aantal_dagen_tot_verval_rte_dag) OVER (partition by contract_oid,peil_dag_dt/100 order by peil_dag_dt rows between 1 following  and 1 following) is null 
           then 	dd.aantal_dagen_tot_verval_rte_dag
           else dd.aantal_dagen_tot_verval_rte_dag - max(dd.aantal_dagen_tot_verval_rte_dag) OVER (partition by contract_oid,peil_dag_dt/100 order by peil_dag_dt rows between 1 following  and 1 following)
      end  as rentedagen_tov_einde_maand     
       
 from 	(select  peil_dag_dt
				,contract_oid
				,opening_dt
				,opgeheven_dt
				,klant_rte_jaar_perc
		...
		where contract_oid = 50484844 			--TEST
		)s
		...
		on s.peil_dag_dt = dd.dim_datum_id 
