...
drop table s_dm_cdo.financiele_transacties;

create table s_dm_cdo.financiele_transacties
as(
sel
trx.bank_nr
...
(select
 ...
 from (select 	row_number() over (partition by trx.contract_oid order by trx.trx_valuta_dt,volg_nr ) as row_nr
		...
		from s_dm_cdo.financiele_transacties trx
...
-- Table met maandstanden en gemiddelde saldi obv waarvan de compensatie per maand en cumulatief wordt berekend
...
drop table s_dm_cdo.rekening_compensatie;

create table s_dm_cdo.rekening_compensatie
as
(
select
b.contract_oid
...
from  s_dm_cdo.rekening_compensatie_gem_saldo a
join S_DM_CDO.rekening_klant_segment b
...
sel * from s_dm_cdo.rekening_compensatie a
join S_DM_VariabeleRente.rente_berekening_agg b
on a.contract_oid = b.contract_oid
