drop table s_dm_variabelerente.klant_rekening_rol;

create table s_dm_variabelerente.klant_rekening_rol
as
(
select cpl.contract_oid	
		...
		,cpl.begin_dt as relatie_begin_dt_dd
		,cpl.relatie_begin_dt
		,cpl.relatie_eind_dt
		,cpl.geldig_ind
...
qualify row_number() over (partition by cpl.contract_oid, cpl.contract_partij_rol_cd order by coalesce(relatie_eind_dt,date'9999-12-31') desc, relatie_begin_dt desc,geldig_ind desc) = 1
) with data unique primary index(contract_oid, partij_oid,contract_partij_rol_cd)
		   

drop table s_dm_variabelerente.klant

create table s_dm_variabelerente.klant
as
(
select   distinct
         cpl.partij_oid
